...
loader(0)
{
   ...
   ImpostorTexture::sceneMgr = group->getParentPagedGeometry()->getSceneManager();
	...
	for (int i = 0; i < IMPOSTOR_PITCH_ANGLES; ++i){
		...
		RTShader::ShaderGenerator::getSingletonPtr()->createShaderBasedTechnique(*material[i][o].get(), Ogre::MaterialManager::DEFAULT_SCHEME_NAME, Ogre::RTShader::ShaderGenerator::DEFAULT_SCHEME_NAME);
